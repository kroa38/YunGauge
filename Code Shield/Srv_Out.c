  ...
  Int8U rtc_nvram_sampling;
        ...
        if (S_Interrupt_Atmega.bRTC_Tic_Int_Flag)     // si interruption RTC TIC 
     {
           ...
           if(Counter_RTC_TIC++ >= COUNTER_TIC_SECONDE)
           {
                ...
                if ( (rtc_nvram_sampling >= 205U) &&  (rtc_nvram_sampling <= 245U) ) // 200 = presence du mode test
                 {
                        rtc_nvram_sampling -= 200U;
                        Counter_RTC_TIC = 60U - rtc_nvram_sampling - 4U;        // on entre toutes les 5 secondes en mode test
